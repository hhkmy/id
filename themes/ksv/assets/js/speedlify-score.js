        ...
        async fetch(speedlifyUrl, url) {
            ...
            if (!this.fetches[speedlifyUrl]) {
                this.fetches[speedlifyUrl] = fetch(
                    SpeedlifyUrlStore.normalizeUrl(
                        speedlifyUrl,
                        "api/urls.json",
                    ),
                );
        ...
        class extends HTMLElement {
            connectedCallback() {
                ...
                if (!this.rawData && !this.speedlifyUrl) {
                    console.log(
                        `Missing \`speedlify-url\` attributes in <speedlify-score>`,
                    );
                    return;
                }
            ...
            getScoreTemplate(data) {
                let scores = [];
                scores.push(
                    `<span title="Performance" class="${this.getScoreClass(data.lighthouse.performance)}">${parseInt(data.lighthouse.performance * 100, 10)}</span>`,
                );
                scores.push(
                    `<span title="Accessibility" class="${this.getScoreClass(data.lighthouse.accessibility)}">${parseInt(data.lighthouse.accessibility * 100, 10)}</span>`,
                );
                scores.push(
                    `<span title="Best Practices" class="${this.getScoreClass(data.lighthouse.bestPractices)}">${parseInt(data.lighthouse.bestPractices * 100, 10)}</span>`,
                );
                scores.push(
                    `<span title="SEO" class="${this.getScoreClass(data.lighthouse.seo)}">${parseInt(data.lighthouse.seo * 100, 10)}</span>`,
                );
                return scores.join(" ");
            }
            ...
            render(data) {
                ...
                if (this.hasAttribute("rank")) {
                    ...
                    content.push(
                        `<${rankUrl ? `a href="${rankUrl}"` : "span"} class="speedlify-rank">${data.ranks.cumulative}</${rankUrl ? "a" : "span"}>`,
                    );
                }
                if (this.hasAttribute("rank-change") && data.previousRanks) {
                    ...
                    content.push(
                        `<span class="speedlify-rank-change ${change > 0 ? "up" : change < 0 ? "down" : "same"}">${change !== 0 ? Math.abs(change) : ""}</span>`,
                    );
                }
